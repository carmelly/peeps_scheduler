...
def create_event(id=1, duration_minutes=120, **kwargs):
...
def create_peep(id=1, role=Role.LEADER, **kwargs):
...
class TestEventAttendeeManagement:
    ...
    def test_add_attendee_success(self):
        ...
        event = create_event()
        peep = create_peep(id=1, role=Role.LEADER)
    ...
    def test_add_attendee_tracks_assignment_order(self):
        """Test that attendee assignment order is preserved."""
        event = create_event()
        peep1 = create_peep(id=1, role=Role.LEADER)
        peep2 = create_peep(id=2, role=Role.FOLLOWER)
    ...
    def test_add_attendee_respects_capacity_limits(self):
        """Test that events reject attendees when at capacity."""
        event = create_event(duration_minutes=60)  # 60min = max 3 per role
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(3)]
    ...
    def test_num_attendees_counts_correctly(self):
        """Test that attendee counts are accurate."""
        event = create_event()
        leader = create_peep(id=1, role=Role.LEADER)
        follower = create_peep(id=2, role=Role.FOLLOWER)
    ...
    def test_clear_participants_removes_all(self):
        """Test that clear_participants removes all attendees and alternates."""
        event = create_event()
        leader = create_peep(id=1, role=Role.LEADER)
        follower = create_peep(id=2, role=Role.FOLLOWER)
        alt = create_peep(id=3, role=Role.LEADER)
...
class TestEventAlternateManagement:
    ...
    def test_add_alternate_success(self):
    ...
    def test_promote_alt_to_attendee(self):
    ...
    def test_demote_attendee_to_alt(self):
    ...
    def test_demote_adds_to_front_of_alternates(self):
        """Test that demoted attendees go to front of alternate list."""
        event = create_event()
        existing_alt = create_peep(id=1, role=Role.LEADER)
        attendee = create_peep(id=2, role=Role.LEADER)
    ...
    def test_remove_alternate_success(self):
...
class TestEventValidation:
    ...
    def test_meets_min_when_sufficient_attendees(self):
        """Test that events with sufficient attendees meet minimums."""
        event = create_event(duration_minutes=120)  # min_role = 6
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(6)]
        followers = [create_peep(id=i+11, role=Role.FOLLOWER) for i in range(6)]
    ...
    def test_does_not_meet_min_when_insufficient(self):
        """Test that events with insufficient attendees don't meet minimums."""
        event = create_event(duration_minutes=120)  # min_role = 6
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(3)]  # Only 3
    ...
    def test_meets_absolute_min_with_basic_attendees(self):
        ...
        for i in range(constants.ABS_MIN_ROLE):
            event.add_attendee(create_peep(id=i+1, role=Role.LEADER), Role.LEADER)
            event.add_attendee(create_peep(id=i+11, role=Role.FOLLOWER), Role.FOLLOWER)
        
        assert event.meets_absolute_min()
    
    def test_does_not_meet_absolute_min_when_insufficient(self):
    ...
    def test_is_full_when_at_capacity(self):
        ...
        event = create_event(duration_minutes=60)  # max_role = 3
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(3)]
    ...
    def test_is_full_overall_when_both_roles_full(self):
...
class TestEventRoleBalancing:
    ...
    def test_balance_roles_with_equal_counts(self):
    ...
    def test_balance_roles_demotes_excess_leaders(self):
        """Test that excess leaders are demoted to alternates."""
        event = create_event()
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(4)]
        followers = [create_peep(id=i+11, role=Role.FOLLOWER) for i in range(2)]
    ...
    def test_balance_roles_demotes_excess_followers(self):
        """Test that excess followers are demoted to alternates."""
        event = create_event()
        leaders = [create_peep(id=i+1, role=Role.LEADER) for i in range(2)]
        followers = [create_peep(id=i+11, role=Role.FOLLOWER) for i in range(5)]
...
class TestEventDurationManagement:
    ...
    def test_downgrade_duration_when_underfilled(self):
        """Test that underfilled events can downgrade duration."""
        event = create_event(duration_minutes=120)  # min_role = 6, max_role = 7, allow_downgrade = True
    ...
    def test_downgrade_duration_fails_when_no_downgrade_allowed(self):
        """Test that downgrade fails when target duration doesn't allow downgrade."""
        event = create_event(duration_minutes=90)  # min_role = 4, max_role = 5
    ...
    def test_downgrade_duration_fails_when_no_valid_option(self):
        """Test that downgrade fails when no valid duration exists."""
        event = create_event(duration_minutes=120)
        
        # Add only 1 per role (below any valid minimum)
        event.add_attendee(create_peep(id=1, role=Role.LEADER), Role.LEADER)
        event.add_attendee(create_peep(id=2, role=Role.FOLLOWER), Role.FOLLOWER)
    ...
    def test_downgrade_duration_raises_if_not_underfilled(self):
    ...
    def test_downgrade_duration_raises_if_unbalanced(self):
...
class TestEventConfiguration:
    ...
    def test_price_per_person_calculation(self):
        ...
        event = create_event(duration_minutes=120)  # $260 total
...
class TestEventAlternateValidation:
    ...
    def test_validate_alternates_removes_ineligible(self):
        """Test that validate_alternates removes alternates who can't attend."""
        event = create_event(id=1)
        
        # Create peep who is available for this event
        eligible_peep = create_peep(id=1, role=Role.LEADER, availability=[1])
        
        # Create peep who is NOT available for this event  
        ineligible_peep = create_peep(id=2, role=Role.LEADER, availability=[2, 3])  # No event 1
    ...
    def test_validate_alternates_removes_over_limit_peeps(self):
        """Test that alternates who exceed event limits are removed."""
        event = create_event(id=1)
        
        # Create peep who has reached their event limit
        over_limit_peep = create_peep(id=1, role=Role.LEADER, availability=[1], event_limit=2)
        over_limit_peep.num_events = 2  # At limit
        
        # Create peep who is under limit
        under_limit_peep = create_peep(id=2, role=Role.LEADER, availability=[1], event_limit=2)
...
class TestEventPeepIntegration:
    ...
    def test_update_event_attendees_updates_stats(self):
        """Test that update_event_attendees correctly updates attendee statistics."""
        event = create_event(id=1)
        
        # Create attendees with initial state
        leader = create_peep(id=1, role=Role.LEADER, priority=3)
        follower = create_peep(id=2, role=Role.FOLLOWER, priority=2)
        non_attendee = create_peep(id=3, role=Role.LEADER, priority=1)
    ...
    def test_update_event_attendees_reorders_list(self):
        """Test that update_event_attendees moves attendees to back of list."""
        event = create_event(id=1)
        
        # Create peeps in specific order
        peep1 = create_peep(id=1, role=Role.LEADER, priority=3)
        peep2 = create_peep(id=2, role=Role.FOLLOWER, priority=2) 
        peep3 = create_peep(id=3, role=Role.LEADER, priority=1)
        peep4 = create_peep(id=4, role=Role.FOLLOWER, priority=0)
    ...
    def test_update_event_attendees_complex_scenario(self):
        """Test update_event_attendees with complex multi-role scenario."""
        event = create_event(id=1)
        
        # Create mixed leaders and followers
        leaders = [create_peep(id=i+1, role=Role.LEADER, priority=i) for i in range(3)]
        followers = [create_peep(id=i+11, role=Role.FOLLOWER, priority=i) for i in range(3)]
