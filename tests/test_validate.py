...
import pytest
from peeps_scheduler.db.validate import (
...
class TestValidatePeriodCancellations:
    ...
    @pytest.mark.parametrize(
        "event_status,expect_issues,expected_msg_contains",
        [
    ...
    def test_cancelled_event_validation(
        ...
        if expect_issues:
            assert len(issues) > 0, (
                f"Expected validation issues but got none (scenario: event_status={event_status})"
            )
            ...
            assert len(issues) == 0, (
                f"Expected no issues but got: {issues} (scenario: event_status={event_status})"
            )

    def test_empty_cancelled_events_list_passes(self, test_db_with_period, test_period_dir):
        """
        ...
        cancellations_data = {"cancelled_events": [], "cancelled_availability": []}
    ...
    def test_multiple_cancelled_events_mixed_states(self, test_db_with_period, test_period_dir):
        """
...
class TestValidatePeriodCancelledAvailability:
    ...
    @pytest.mark.parametrize(
        "create_availability_record,expect_issues",
        [
    ...
    def test_cancelled_availability_validation(
        ...
        issues = validate_period_cancelled_availability(
            cursor, period_id, cancellations_data, period_name
        )

        # Assert based on expected outcome
        if expect_issues:
            assert len(issues) > 0, (
                f"Expected issues when availability not removed (create_availability_record={create_availability_record})"
            )
            assert any(
                "not removed" in issue.lower() or "still has availability" in issue.lower()
                for issue in issues
            ), f"Expected 'not removed' or 'still has availability' issue, got: {issues}"
        else:
            assert len(issues) == 0, (
                f"Expected no issues when availability correctly removed, got: {issues}"
            )
    ...
    def test_empty_cancelled_availability_list_passes(self, test_db_with_period, test_period_dir):
        """
    ...
    def test_multiple_cancelled_availability_entries_mixed_states(
        ...
        assert len(issues) == 1, f"Expected 1 issue (still exists), got {len(issues)}: {issues}"
...
class TestValidatePeriodPartnerships:
    ...
    @pytest.mark.parametrize(
        "create_partnerships,expect_issues",
        [
    ...
    def test_partnership_existence_validation(
        ...
        partnerships_data = {"1": [2], "2": [1] if create_partnerships else []}
        ...
        if expect_issues:
            assert len(issues) > 0, (
                f"Expected issues when partnerships not in DB (create_partnerships={create_partnerships})"
            )
            ...
            assert len(issues) == 0, (
                f"Expected no issues when partnerships exist in DB, got: {issues}"
            )

    @pytest.mark.parametrize(
        "invalid_field,partnerships_data,expected_keyword",
        [
    ...
    def test_invalid_id_format_validation(
        ...
        assert any(
            "invalid" in issue.lower() and expected_keyword in issue.lower() for issue in issues
        ), f"Expected 'invalid' and '{expected_keyword}' in issue, got: {issues}"

    @pytest.mark.parametrize(
        "format_type,partnerships_data",
        [
            ("wrapped", {"partnerships": {"1": [2]}}),  # Wrapped format
            ("unwrapped", {"1": [2]}),  # Unwrapped format
        ],
    )
    def test_json_format_validation(self, test_db_with_period, format_type, partnerships_data):
        """
        ...
        assert len(issues) == 0, (
            f"validate_period_partnerships() should work with {format_type} format. Issues found: {issues}"
        )

    def test_empty_partnerships_passes(self, test_db_with_period, test_period_dir):
        """
    ...
    def test_multiple_partners_per_requester(self, test_db_with_period, test_period_dir):
        """
    ...
    def test_multiple_partnerships_mixed_states(self, test_db_with_period, test_period_dir):
        """
        ...
        assert len(issues) == 1, f"Expected 1 issue (not found), got {len(issues)}: {issues}"
