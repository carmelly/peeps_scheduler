...
def apply_migration(filepath, conn):
    ...
    try:
        ...
        conn.execute("INSERT INTO __migrations_applied__ (filename) VALUES (?)", (filename,))
        ...
        with suppress(Exception):  # Rollback might fail if transaction wasn't started
            conn.execute("ROLLBACK")
        raise Exception(f"Database error in {filename}") from e
    except Exception as e:
        # Rollback on any other error
        with suppress(Exception):
            conn.execute("ROLLBACK")
        raise Exception(f"Error applying {filename}") from e
    ...
    try:
        ...
        for i, filename in enumerate(pending_files, 1):
            ...
            try:
                ...
                print("Fix the migration and run again.")
                return False
        ...
        try:
            schema_output = subprocess.check_output(
                [SQLITE_EXE, str(DB_PATH), ".schema"], text=True
            )
