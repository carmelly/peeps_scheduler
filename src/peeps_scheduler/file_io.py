...
def load_csv(filename, required_columns=None, optional_columns_defaults=None):
    """
...
def load_responses(response_csv_path):
    ...
    optional_defaults = {"Secondary Role": ""}
    ...
    return load_csv(
        ...
        optional_columns_defaults=optional_defaults,
    )
...
def load_cancelled_events(data_folder, year=None):
    ...
    for event_str in cancelled_event_strings:
        try:
            ...
            raise ValueError(
                f"cannot parse event string in cancelled_events.json: '{event_str}' - {e}"
            ) from e
...
def load_partnerships(data_folder, partnerships_filename="partnerships.json", valid_peep_ids=None):
    ...
    raw_requests = (
        data.get("partnerships") if isinstance(data, dict) and "partnerships" in data else data
    )
    ...
    def coerce_id(value, context=""):
        try:
            ...
            raise ValueError(f"invalid {context} id '{value}': must be an integer") from e
    ...
    for requester_key, partners in raw_requests.items():
        ...
        if not isinstance(partners, list):
            raise ValueError(
                f"partner list for requester {requester_id} must be a list, got {type(partners).__name__}"
            )
        ...
        for partner in partners:
            ...
            if valid_peep_ids is not None and partner_id not in valid_peep_ids:
                raise ValueError(
                    f"partnership partner id {partner_id} for requester {requester_id} not found in peeps"
                )
...
def load_data_from_json(filename):
    ...
    if not Peep.is_peeps_list_sorted_by_priority(sorted_peeps):
        raise ValueError(
            "peeps data is not sorted by priority (highest to lowest). check input file data integrity."
        )
...
def load_cancellations(filename, year=None):
    ...
    for event_str in cancelled_event_strings:
        ...
        try:
            ...
            raise ValueError(
                f"invalid cancelled_events entry: '{event_str}' does not match expected format (e.g., 'Friday February 7th - 5pm to 7pm')"
            ) from e
    ...
    for entry in cancelled_availability_entries:
        ...
        for event_str in event_strings:
            if not isinstance(event_str, str) or not event_str.strip():
                raise ValueError(
                    f"cancelled_availability event for {email} must be a non-empty string"
                )
            try:
                event_id, _, _ = parse_event_date(event_str, year=year)
            except Exception as e:
        ...
        for row in event_rows:
            ...
            if duration_str:
                try:
                    ...
                    raise ValueError(f"invalid event duration value: {duration_str}") from e
        ...
        for row in rows:
            ...
            for date_str in date_strings:
                ...
                if duration is None:
                    duration = 120
                    logging.debug(
                        f"No duration in event string '{date_str}', defaulting to {duration} minutes"
                    )

                if duration not in constants.CLASS_CONFIG:
                    raise ValueError(
                        f"Duration {duration} minutes not in CLASS_CONFIG (valid: {list(constants.CLASS_CONFIG.keys())})"
                    )
                ...
                if event_id not in unique_events:
                    ...
                    if existing_duration != duration:
                        raise ValueError(
                            f"Inconsistent duration for event {event_id}: {existing_duration} vs {duration}"
                        )

        # Create Event objects from unique events
        for event_id, (duration, _display_name) in sorted(unique_events.items()):
            event = Event.from_dict(
                {
                    "id": event_counter,
                    "date": event_id,
...
def process_responses(rows, peeps, event_map, year=None):
    ...
    for row in rows:
        ...
        if not peep.active:
            raise ValueError(
                f"response from inactive peep: {peep.full_name} (ID {peep.id}) — please activate them in the members spreadsheet."
            )
...
def parse_event_date(date_str, year=None):
    ...
    if has_time_range:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"invalid event date format (expected 'Date - Time Range'): {date_str}"
            )
        ...
        try:
            ...
            raise ValueError(f"invalid date format in '{date_str}'") from e
