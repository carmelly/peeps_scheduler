...
class Peep:
    ...
    def __repr__(self):
        return (
            f"Peep(id={self.id}, name='{self.name}', priority={self.priority}, "
    ...
    def __str__(self):
        ...
        return (
            f"Peep({self.id:>3}): p: {self.priority}, limit: {self.event_limit}, "
...
class Event:
    ...
    def remove_alternate(self, peep: Peep, role: Role):
        ...
        if peep not in alt_list:
            logging.error(
                f"Peep {peep.id} not in alt list for role {role.value} in Event {self.id}"
            )
    ...
    def meets_absolute_min(self) -> bool:
        ...
        """
        return (
            len(self.leaders) >= constants.ABS_MIN_ROLE
            and len(self.followers) >= constants.ABS_MIN_ROLE
        )
    ...
    def is_full(self, role: Role = None) -> bool:
        ...
        if count > self.max_role:
            logging.error(
                f"Too many {role.value}s in Event {self.id}: {count} > max of {self.max_role}"
            )
    ...
    def balance_roles(self):
        ...
        if len(self.leaders) != len(self.followers):
            logging.error(
                f"Role imbalance after balancing for Event {self.id}: {len(self.leaders)}L / {len(self.followers)}F"
            )
    ...
    def downgrade_duration(self) -> bool:
        ...
        if len(self.leaders) != len(self.followers):
            logging.error(
                f"Cannot downgrade Event {self.id}: roles are unbalanced ({len(self.leaders)}L / {len(self.followers)}F)"
            )
        ...
        logging.debug(
            f"Attempting to downgrade Event {self.id} due to underfill ({count_per_role}/role)"
        )
        ...
        for duration in sorted(constants.CLASS_CONFIG.keys()):
            ...
            if (
                new_config["allow_downgrade"]
                and new_config["min_role"] <= count_per_role <= new_config["max_role"]
            ):
                logging.debug(
                    f"Downgrading Event {self.id} to {duration} minutes (was {self.duration_minutes})"
                )
                ...
                if len(self.leaders) < self.min_role:
                    logging.error(
                        f"Too few attendees after balancing for Event {self.id}: {len(self.leaders)} per role, minimum required is {self.min_role}"
                    )
                    raise RuntimeError(
                        f"Event {self.id} has too few attendees after balance_roles()"
                    )

                return True

        logging.warning(
            f"No valid downgrade found for Event {self.id} with {count_per_role} per role"
        )
        return False
    ...
    def validate_alternates(self):
        ...
        for role in (Role.LEADER, Role.FOLLOWER):
            for peep in self.get_alternates(role)[
                :
            ]:  # copy list to avoid mutation during iteration
                if not peep.can_attend(self):
                    logging.debug(
                        f"Removing ineligible alternate {peep.name} from Event {self.id} ({role.value})"
                    )
    ...
    def __repr__(self):
        ...
        return (
            f"Event(event_id={self.id}, date={self.date}, "
    ...
    def get_participants_str(self, role: Role = None) -> str:
        ...
        def format_group(role: Role) -> str:
            ...
            name_str = ", ".join(
                f"*{peep.name}" if peep.role != role else peep.name for peep in attendees
            )
            alt_str = ", ".join(
                f"*{peep.name}" if peep.role != role else peep.name for peep in alternates
            )
...
class EventSequence:
    ...
    def __str__(self):
        ...
        unassigned = [
            p.name for p in self.peeps if p.responded and p.availability and p.num_events == 0
        ]
