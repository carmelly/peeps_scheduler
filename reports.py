...
def list_peeps_by_snapshot():
	with DbManager(DB_PATH) as db: 
		...
		for p in sorted(snapshot_peeps, key=lambda x: x["index"]):
			...
			if not peep:
				continue
			print(f"  {p['index']:>2}. {peep['display_name']} ({peep['primary_role']})  id={peep['id']}  priority={p['priority']}")
...
def availability_report():
	...
	for date in sorted(availability.keys()):
		formatted_date = datetime.strptime(date, DATE_FORMAT).strftime("%A %b %d %I:%M %p")
